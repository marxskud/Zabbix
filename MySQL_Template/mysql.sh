...
if [ "$1" == "max_connections" ]; then
  MYSQLV=`echo "SHOW GLOBAL VARIABLES" | /usr/bin/mysql --defaults-file=/etc/zabbix/debian.cnf`
...
#send status (once per 5min by default)
  ...
  MYSQLS=`echo "SHOW GLOBAL STATUS" | /usr/bin/mysql --defaults-file=/etc/zabbix/debian.cnf`
  for stat in ${status[*]}; do
    echo "$MYSQLS" | grep "^$stat\s" >> $TMPS
  done
  echo "SHOW SLAVE STATUS\G" | /usr/bin/mysql --defaults-file=/etc/zabbix/debian.cnf | grep Seconds_Behind_Master | sed -r 's/^ *//' | sed 's/://' | sed 's/NULL/0/' >> $TMPS
...
#number of transactions with rows_locked >X (default 10000)
  ...
  echo "select count(*) from INFORMATION_SCHEMA.INNODB_TRX where trx_rows_locked>$t" | /usr/bin/mysql --defaults-file=/etc/zabbix/debian.cnf | tail -n 1
...
#number of transactions started > X sec ago
  ...
  echo "select count(*) from INFORMATION_SCHEMA.INNODB_TRX $w" | /usr/bin/mysql --defaults-file=/etc/zabbix/debian.cnf | tail -n 1

#number of transactions with query running > X sec
elif [ "$1" == "trans-running" ]; then
  if [ "$2" == "" ]||[ "$2" == "0" ]; then w=''; else w="AND p.TIME>$2"; fi
  echo "select count(*) from INFORMATION_SCHEMA.INNODB_TRX t, INFORMATION_SCHEMA.PROCESSLIST p WHERE t.trx_mysql_thread_id=p.ID AND p.COMMAND='Query' $w" | /usr/bin/mysql --defaults-file=/etc/zabbix/debian.cnf | tail -n 1
